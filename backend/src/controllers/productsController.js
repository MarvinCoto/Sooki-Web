...
productsController.getById = async (req, res) => {
  try {
    ...
    const product = await productModel
      .findOne({ _id: id, idStore: storeId })
      .populate("idCategory", "name");
...
productsController.update = async (req, res) => {
  try {
    const storeId = req.user.id;
    const { id } = req.params;
    const { name, description, idCategory, discount, status, keepImages } =
      req.body;
...
productsController.toggleStatus = async (req, res) => {
  try {
    ...
    if (!product.status) {
      ...
      if (!hasStock) {
        return res.status(400).json({
          message:
            "No se puede activar. El producto no tiene variantes con stock disponible.",
        });
      }
    }
    ...
    res
      .status(200)
      .json({ message: "Product status updated", status: product.status });
  } catch (error) {
    res
      .status(500)
